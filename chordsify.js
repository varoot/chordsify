    ...
    Chords.prototype.init = function(key, text) {
      ...
      $.each(text.trim().split("\n"), function(i, lineText) {
        ...
        return $.each(lineText.match(/\s*([^\[\]\s]+|\[[^\]]*\])+\s*/g), function(j, wordText) {
          ...
          return $.each(wordText.split('['), function(k, chunkText) {
            var $chordWrapper, $chunk, $lyrics, chordText, lyricsText, _ref;
            if (chunkText === '') {
              return;
            }
            $chunk = $(opts.elements.chunk).addClass(opts.classes.chunk).appendTo($word);
            if (chunkText.indexOf(']') < 0) {
              lyricsText = chunkText;
            } else {
              _ref = chunkText.split(']'), chordText = _ref[0], lyricsText = _ref[1];
              $chordWrapper = $(opts.elements.chordWrapper).addClass(opts.classes.chordWrapper).html(makeChordElement(chordText, key, opts)).appendTo($chunk);
              if (lyricsText === '') {
                lyricsText = ' ';
              }
            }
            return $lyrics = $(opts.elements.lyrics).addClass(opts.classes.lyrics).text(lyricsText).appendTo($chunk);
          });
        });
      });
