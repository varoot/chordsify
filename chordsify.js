...
(function($) {
  var Chords, chordsRegEx, flatKeys, key, keyMap, keyNumber, makeChordElement, relativeKey, replaceFlatSharp, restoreFlatSharp, sharpKeys, validKey;
    ...
    $element.find('.' + opts.classes.lyrics + ', .' + opts.classes.chord).each(function(i, phrase) {
      ...
      if ($phrase.hasClass(opts.classes.chord)) {
        ...
        if (offs.top === lastChordTop && offs.left < lastChordRight) {
          $gap = $(opts.elements.gap).addClass(opts.classes.gap);
          if (lastGapDash) {
  ...
  Chords.prototype.init = function(key, text) {
    ...
    $.each(text.trim().split("\n"), function(i, lineText) {
      ...
      $line = $(opts.elements.line).addClass(opts.classes.line).appendTo($block);
      if (lineText === "") {
        $line.text(" ");
        return;
      }
  ...
  Chords.prototype.transpose = function(key) {
    var $element, chordKeys, k, opts, self;
    self = this;
    $element = $(this.element);
    opts = this.options;
    key = keyNumber(key);
